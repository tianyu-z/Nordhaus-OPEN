* Produces a file "DICE2023-b-4-3-10.csv" in the base directory
...
growthAL(t)
drshort(t) ;
gpccons(t+1)=log(cpc.l(T+1)/cpc.l(t))/5;
drshort(t+1) = rshort.l(t+1)-rshort.l(t) ;
growthAL(t+1)=log((AL(t+1)/AL(t)))/5;
...
Loop (T, put Y.l(T));
put / "Short Interest rate, %/yr" ;
Loop (T, put Rshort.l(T));
put / "Population" ;
Loop (T, put L(T));
put / "TFP" ;
Loop (T, put AL(T));
...
put / "Growth p.c. cons (log)"
Loop (T, put gpccons(t) );
put / "Change interest rate"
Loop (T, put drshort(t) );
put / "Precautionary effect"
Loop (T, put (rprecaut(t)) );
put / "Long interest rate(%)"
Loop (T, put (rlong.l(t)) );
put / "RR1"
Loop (T, put (rr1(t)) );
put / "RR"
Loop (T, put (rr(t)) );
...
put / "Variance pc cons"
Loop (T, put (varpcc(t))) ;
put / "Growth TFP"
Loop (T, put (growthAL(t+1))) ;
